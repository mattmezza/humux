# Web artifacts — publish a page with the write tool
...
(`write`, `edit`). The server exposes that directory at
`/artifacts/<slug>/` automatically.

This needs the workspace harness to be enabled (the `write` tool). If you
don't have `write`, you can't publish artifacts — tell the user to enable
...
```
write(path="artifacts/q3-report/index.html", content="<!doctype html><html>…</html>")
```
...
```
write(path="artifacts/dash/index.html", content="<link href='style.css'>…<script src='app.js'></script>")
write(path="artifacts/dash/style.css", content="body{font-family:system-ui}")
write(path="artifacts/dash/app.js", content="console.log('hi')")
```
...
via `bash`, output into `artifacts/<slug>/`), then it's served
...
- **No TTL / expiry.** Artifacts live until you remove them. Overwrite a slug to
  update it; use `ls artifacts/` via bash to see what's published; remove an old
  one by clearing its directory with `bash` (`rm -rf artifacts/<slug>`)
...
- Writing a file asks the owner for approval (the standard `write` prompt),
