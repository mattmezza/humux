...
| GitHub | `gh.md` | Issues, PRs, releases and `gh api` via the `gh` CLI |
| Browser | `browser.md` | Headless Playwright browsing and page actions |
| Image generation | `image_generation.md` | `generate_image` conventions |
| Artifact hosting | `artifact-hosting.md` | Publishing [web artifacts](/docs/artifacts) |
| Subagent orchestration | `subagent-orchestration.md` | When to fan work out to parallel [subagents](/docs/subagents), and how to brief and reassemble them |
...
The index is names + one-line summaries only. The instruction to read a skill
before acting on it is stated once, in the cached `<tool_usage>` block, rather
than repeated in the index and in each tool's prompt block.
