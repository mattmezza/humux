...
Core agent settings including name, owner, LLM provider, model, and timezone. The `skills_dir` points to the directory containing markdown skill files. `agents_dir` is an optional seed directory for [agents](/docs/agents) — empty by default (no agents ship), but any `.md` files dropped there are auto-seeded. Which agent is the default (the one that answers when nothing else routes) is a per-row flag, not a config key — set it on the Agents tab.
