...
Core agent settings including name, owner, LLM provider, model, and timezone. `max_tool_rounds` (default `100`) caps how many tool-call rounds one turn may take before the loop is cut off — a runaway-loop rail, not a working limit; it is exposed on the admin **LLM** tab. The `skills_dir` points to the directory containing markdown skill files. `agents_dir` is an optional seed directory for [agents](/docs/agents) — empty by default (no agents ship), but any `.md` files dropped there are auto-seeded. Which agent is the default (the one that answers when nothing else routes) is a per-row flag, not a config key — set it on the Agents tab.
...
The [coding harness](/docs/workspace) — file `read`/`write`/`edit` tools confined to `directory`, plus workspace-confined execution of non-allowlisted `bash` commands. Reads are pre-approved; writes and commands ask for approval each time. Paths escaping `directory` (via `..` or a symlink) are blocked. Off by default, and managed from the admin **Workspace** tab; a blank `directory` keeps the tools inert even when `enabled` is true — there is no default root.

#### `subagents`

[Subagent](/docs/subagents) delegation and its guardrails. `enabled` is the
master switch; the rest bound what a spawn may cost:
...
subagents:
  ...
  token_budget: 300000        # approx token ceiling per run
  max_concurrent: 4           # per-chat concurrency gate; extras queue
  max_fanout: 6               # max tasks in one spawn_subagents call
  max_spawns_per_turn: 12     # spawns across the whole tree, per turn
  turn_token_budget: 1200000  # subagent tokens across the whole tree, per turn
  allowed_models: []          # e.g. ["deepseek:deepseek-v4-flash"]
```

Every key is also editable live from the admin **Tools** tab (the *Subagents*
card). See [Subagents → Guardrails](/docs/subagents#guardrails) for what each one
does and how the agent may dial it down per spawn.

#### `reply_decision`

In a shared/group chat with several bots and people, decide per message whether to reply at all — staying quiet for messages aimed at another bot, caught in a bot-to-bot reaction loop, or that the agent can't usefully add to. A cheap one-shot LLM call (`model`) makes the call, fed the recent conversation (so follow-ups and pronouns read in context) and the names of the sibling bots in the room (so "aimed at another bot" is anchored to real agents); it **fails open** (replies) on any error, so a classifier hiccup never drops a real message. Off by default — 1:1 chats always reply and it costs one extra call. `group_only` (default `true`) gates only group chats (detected when the chat id differs from the user id). `max_replies_per_window`/`window_seconds` are a hard per-chat rate cap that backstops the LLM gate, so a runaway loop terminates even if the gate keeps voting "reply". Scheduler/system turns are never gated.
