...
"spawn_subagent"                     # Delegating a subtask
"spawn_subagents"                    # Delegating a parallel fan-out
```

The [subagent](/docs/subagents) prompts name the target agent and the task, and
they are the *only* human gate on the whole run — a subagent runs with system
semantics, without per-action approval.
...
- **Telegram Bot API is off limits to the shell** — any model-issued command targeting `api.telegram.org` (or carrying a `/bot<id>:<token>` path) is rejected on both shell rails, so the bot token being readable from config can't turn `curl` into an unguarded `pinChatMessage`/`deleteMessage`/`sendMessage`. The structured `set_reaction` and `send_message` tools, which do honour permissions and per-chat settings, are the only way in
