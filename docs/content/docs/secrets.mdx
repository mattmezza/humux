...
| Used as | `${vault:NAME}` in config (with `.env` fallback) | `{{secret:NAME}}` inside `bash` |
...
An agent references a secret by name inside the `bash` tool only:

```
bash("curl -H 'Authorization: Bearer {{secret:STRIPE_KEY}}' https://api.stripe.com/...")
```
...
Substitution happens **only** at the `bash` boundary (allowlisted commands), after an ACL check —
...
An agent may use a secret if it is **shared** (global) or its name is in the agent's
...
to `bash`; per-agent scoping; the agent key never written to disk; audit records
