...
- **Vaults** — the [secrets vault](/docs/secrets), split into **Agents** (per-persona / login secrets), **Infra** (machine-key-encrypted infrastructure secrets + migrate plaintext credentials into the vault), and **Import** (Bitwarden import)
...
Manage the [secrets vault](/docs/secrets), organized into three sub-tabs: **Agents** (per-persona
/ login secrets — a **persona × secret access matrix** to grant a persona use of a secret, plus
